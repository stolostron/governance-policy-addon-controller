...
func debugCollection(ctx context.Context, podSelector string) {
	...
	for i, cluster := range managedClusterList {
		...
		if cluster.clusterName == "cluster1" {
			...
			stringBuilder.WriteString(Kubectl(ctx, "get", "clustermanagementaddons", "-o=yaml", targetKubeconfig))
			stringBuilder.WriteString("---\n")
			stringBuilder.WriteString(Kubectl(ctx, "get", "managedclusteraddons", "-A", "-o=yaml", targetKubeconfig))
			stringBuilder.WriteString("---\n")
			stringBuilder.WriteString(Kubectl(ctx, "get", "addondeploymentconfigs", "-A", "-o=yaml", targetKubeconfig))
			stringBuilder.WriteString("---\n")
			stringBuilder.WriteString(Kubectl(ctx, "get", "manifestwork", "-A", "-o=yaml", targetKubeconfig))
		...
		for _, namespace := range clusterNs {
			for _, suffix := range namespaceSuffix {
				...
				stringBuilder.WriteString(Kubectl(ctx, "get", "all", "-n", namespace, targetKubeconfig))
				stringBuilder.WriteString("::endgroup::\n")

				fmt.Fprintf(&stringBuilder, "::group::Cluster %s: Pod logs for label %s in namespace %s:\n",
					targetCluster, podSelector, namespace)
				stringBuilder.WriteString(
					Kubectl(ctx, "describe", "pod", "-n", namespace, "-l", podSelector, targetKubeconfig))
				stringBuilder.WriteString(
					Kubectl(ctx, "logs", "-n", namespace, "-l", podSelector, "--ignore-errors", targetKubeconfig))
				stringBuilder.WriteString("::endgroup::\n")
			}
		}
		...
		for _, namespace := range deploymentNamespaces {
			...
			stringBuilder.WriteString(Kubectl(ctx, "describe", "pod", "-n", namespace, "-l", podSelector,
				targetKubeconfig))
			stringBuilder.WriteString(
				Kubectl(ctx, "logs", "-n", namespace, "-l", podSelector, "--ignore-errors", targetKubeconfig))
			stringBuilder.WriteString("::endgroup::\n")
