...
var _ = Describe("Test framework deployment", func() {
	AfterEach(func(ctx SpecContext) {
		if CurrentSpecReport().Failed() {
			debugCollection(ctx, case1PodSelector)
		}

		By("Restoring the default governance-policy-framework ClusterManagementAddon on the hub cluster")
		Kubectl(ctx, "apply", "-f", case1ClusterManagementAddOnCRDefault)

		By("Cleaning up CSRs")
		Kubectl(ctx, "delete", "csr", "-l", "open-cluster-management.io/addon-name="+case1ManagedClusterAddOnName)
	})
	...
	It("should create the framework deployment in user's custom namespace", func(ctx SpecContext) {
		...
		Kubectl(ctx, "apply", "-f", addOnDeploymentConfigWithAgentInstallNs)
		DeferCleanup(func(ctx SpecContext) {
			By("Delete the AddOnDeploymentConfig")
			Kubectl(ctx, "delete", "-f", addOnDeploymentConfigWithAgentInstallNs)
		})

		By("Applying the framework ClusterManagementAddOn to use the AddOnDeploymentConfig")
		Kubectl(ctx, "apply", "-f", case1CMAAddonWithInstallNs)

		for _, cluster := range managedClusterList {
			logPrefix := cluster.clusterType + " " + cluster.clusterName + ": "
			By(logPrefix + "deploying the default framework managedclusteraddon")
			Kubectl(ctx, "apply", "-n", cluster.clusterName, "-f", case1ManagedClusterAddOnCR)
			...
			Kubectl(ctx, "delete", "-n", cluster.clusterName, "-f", case1ManagedClusterAddOnCR, "--timeout=180s")
			deploy = GetWithTimeout(
	...
	It("should create the default framework deployment in hosted mode in klusterlet agent namespace",
		Label("hosted-mode"), func(ctx SpecContext) {
			By("Creating the AddOnDeploymentConfig")
			Kubectl(ctx, "apply", "-f", addOnDeploymentConfigWithManagedKubeconfigCR)
			By("Applying the governance-policy-framework ClusterManagementAddOn to use the AddOnDeploymentConfig")
			Kubectl(ctx, "apply", "-f", case1ClusterManagementAddOnCR)
			...
			Kubectl(ctx, "delete", "-f", addOnDeploymentConfigWithManagedKubeconfigCR, "--timeout=15s")
		})
	...
	It("should create a framework deployment with customizations from annotations", func(ctx SpecContext) {
		for i, cluster := range managedClusterList {
			...
			Kubectl(ctx, "annotate", "-n", cluster.clusterName, "-f", case1ManagedClusterAddOnCR,
			...
			Kubectl(ctx, "annotate", "-n", cluster.clusterName, "-f", case1ManagedClusterAddOnCR, clientQPSAnnotation)
	...
	It("should create a framework deployment with customizations from AddOnDeploymentConfig", func(ctx SpecContext) {
		By("Creating the AddOnDeploymentConfig")
		Kubectl(ctx, "apply", "-f", addOnDeploymentConfigWithCustomVarsCR)
		By("Applying the governance-policy-framework ClusterManagementAddOn to use the AddOnDeploymentConfig")
		Kubectl(ctx, "apply", "-f", case1CMAAddonWithCustomizedVars)
		...
		By("Deleting the AddOnDeploymentConfig")
		Kubectl(ctx, "delete", "-f", addOnDeploymentConfigWithCustomVarsCR, "--timeout=15s")
	})

	It("should create a framework deployment with node selector on the managed cluster", func(ctx SpecContext) {
		By("Creating the AddOnDeploymentConfig")
		Kubectl(ctx, "apply", "-f", addOnDeploymentConfigCR)
		By("Applying the governance-policy-framework ClusterManagementAddOn to use the AddOnDeploymentConfig")
		Kubectl(ctx, "apply", "-f", case1ClusterManagementAddOnCR)
		...
		By("Deleting the AddOnDeploymentConfig")
		Kubectl(ctx, "delete", "-f", addOnDeploymentConfigCR, "--timeout=15s")
	})
	...
	It("should create a framework deployment with resource requirements on the managed cluster",
		func(ctx SpecContext) {
			...
			for configFile, expected := range deploymentConfigTests {
				By("Creating the AddOnDeploymentConfig")
				Kubectl(ctx, "apply", "-f", configFile)
	...
	It("should use the onManagedClusterHub value set in helm values annotation", func(ctx SpecContext) {
		...
		Kubectl(ctx, "apply", "-n", cluster.clusterName, "-f", case1hubValuesMCAOCR)
		...
		By(logPrefix + "annotating the managedclusteraddon with the " + loggingLevelAnnotation + " annotation")
		Kubectl(ctx, "annotate", "-n", cluster.clusterName, "-f", case1ManagedClusterAddOnCR, loggingLevelAnnotation)
	...
	It("should use the onMulticlusterHub value set in the custom annotation on the ManagedCluster object",
		func(ctx SpecContext) {
			...
			Kubectl(ctx, "annotate", "-n", cluster.clusterName, "-f", case1ManagedClusterAddOnCR, "policy-addon-pause-")
	...
	It("should use the onMulticlusterHub value set in the custom annotation", func(ctx SpecContext) {
		...
		By(logPrefix + "deploying the annotated framework managedclusteraddon")
		Kubectl(ctx, "apply", "-n", cluster.clusterName, "-f", case1hubAnnotationMCAOCR)
	...
	It("should revert edits to the ManifestWork by default", func(ctx SpecContext) {
		for _, cluster := range managedClusterList {
			...
			Kubectl(ctx, "patch", "-n", cluster.clusterName, "manifestwork", case1MWName, "--type=json",
